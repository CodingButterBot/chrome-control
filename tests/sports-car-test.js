...
const mcpClient = {
  request: async (method, params = {}) => {
    ...
    // Execute the command through the stdio adapter
    const command = `echo '${JSON.stringify(request)}' | node ${projectRoot}/bin/stdio.js`;
    console.log(`Running command: ${command}`);
    
    const response = execSync(command, { encoding: 'utf8', maxBuffer: 10 * 1024 * 1024 });
